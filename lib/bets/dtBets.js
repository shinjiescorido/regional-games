	...
	processBets (args) {
		...
		if (_.isEmpty(result.bets)) {
			...
			.then(() => {
				...
				this.sendToAPIServer('Dragon-Tiger', result, [], dealerName);
		...
		.then(() => {
			result.gameResult = gameResult;
			this.sendToAPIServer('Dragon-Tiger', result, apiUserInfo, dealerName);
			 return {roundWins,roundNum,userInfo};
		})
		.catch(e => {
			t.rollback();
